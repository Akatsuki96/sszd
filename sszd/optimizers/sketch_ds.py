...
class SketchDS(Optimizer):
    ...
    def __init__(self, target : Callable[[torch.Tensor, Optional[Any]], torch.Tensor], 
                 ...
                 ds_constant : float, 
                 alpha_lims,
                 P : DirectionMatrix, # direction matrix
                 G : DirectionMatrix # sketching matrix
                 ):
        super().__init__('sketch_ds', target)
        self.alpha = alpha_0
        ...
        while k < T:
           # k += 1
            ...
            for i in range(self.D.shape[1]):
                d = G @ self.D[:, i].flatten()
                f_next = self.target(x_k + self.alpha * d, z_k)
                k += 1
                nev += 1
                if f_next-  f_k <  - self.ds_constant * (self.alpha**2):
