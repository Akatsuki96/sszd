...
def plot_results(results):
   ...
   # ax1.set_title("Function values")
    ...
    ax1.legend()
    plt.savefig("nonconv_vals.png", bbox_inches="tight")
    plt.close(fig)
    ...
    ax2.set_yscale("log")
    ax2.legend()
    plt.savefig("nonconv_time.png", bbox_inches="tight")
    plt.close(fig)
...
def gds_experiment(target, init_alpha, options, reps=5):
    ...
    for i in range(reps):
        ...
        for k in range(1, T):
            ...
            it_time = time.time() - it_time
            print("[{}] i: {}/{}\tk: {}/{}\t F(x, theta) = {}\tf(x) = {}".format(optimizer.options.gen_strat,i, reps, k, T, target(x, theta), target.complete(x)))
            result.append_result(i, k, it_time, target.complete(x), target(x, theta))
        optimizer.reset()
    return result
