...
def plot_results(results):
   ...
   # ax1.set_title("Function values")
    ax1.set_title("Stochastic function values: PL Convex Function", fontsize=20)
    ...
    ax1.legend()
    plt.savefig("pl_conv_vals.png", bbox_inches="tight")
    plt.close(fig)
    ...
    ax2.set_yscale("log")
    ax2.legend()
    plt.savefig("pl_conv_time.png", bbox_inches="tight")
    plt.close(fig)
...
def stp_experiment(target, init_alpha, options, reps=5):
    ...
    for i in range(reps):
        ...
        for k in range(T):
            ...
            print("[--] i: {}/{}\tk: {}/{}\t F(x, theta) = {}\tf(x) = {}".format(i, reps, k, T, target(x, theta), target.complete(x)))
...
#comp_options = GDSOptions(d, alpha_max=100.0, exp_factor=2.0, cont_factor=0.5, forcing_fun=forc_fun, gen_strat='compass')
...
nhalf_indep = GDSOptions(d, alpha_max=10.0, exp_factor=2.0, cont_factor=0.5, gen_strat="n_half", forcing_fun=forc_fun)
...
target = SquareNormPL(d)

#comp = gds_experiment(target, 5., comp_options, reps=reps)
