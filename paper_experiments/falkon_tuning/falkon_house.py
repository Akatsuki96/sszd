...
def stp_experiment(Xtr, Xte, ytr, yte, options, reps=5):
    ...
    for i in range(reps):
        ...
        init_lam = init_config_state.rand(1) * (1e-5 - 1e-6) + 1e-6
...
def szo_experiment(Xtr, Xte, ytr, yte, directions, l, reps=5, mname="SZO"):
    d = Xtr.shape[1] 
    assert l <= d
    alpha = lambda k:   10 * 1/(np.sqrt(k)) #* 1e-1
    h = lambda k :  1e-2 #* 1e-1
...
comp_options = GDSOptions(9, alpha_max = 5.0, exp_factor=2, cont_factor=0.5, gen_strat="compass")
stp_options = GDSOptions(9, gen_strat="random_unit")
sketch_unit_options = GDSOptions(9, alpha_max = 5.0, exp_factor=2, cont_factor=0.5, gen_strat="random_unit", sketch=("gaussian", l))
sketch_orth_options = GDSOptions(9, alpha_max = 5.0, exp_factor=2, cont_factor=0.5, gen_strat="random_orth", sketch=("orthogonal", l))
unit_options = GDSOptions(9, alpha_max = 5.0, exp_factor=2, cont_factor=0.5, gen_strat="random_unit")
orth_options = GDSOptions(9, alpha_max = 5.0, exp_factor=2, cont_factor=0.5, gen_strat="random_orth")
n_half_options = GDSOptions(9, alpha_max = 5.0, exp_factor=2, cont_factor=0.5, gen_strat="n_half")


szo_sph_ris, te_err = szo_experiment(Xtr, Xte, ytr, yte, "spherical", l, reps=reps, mname="SZO-SP")
