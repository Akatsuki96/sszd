...
def plot_results(results, title, y_ax, out_file_name, ylim=None, logscale=False):
    ...
    for (val_err_mean, val_err_std, label, color) in results:
        ax.plot(range(val_err_mean.shape[0]), val_err_mean, '-', c="{}".format(color), label="{}".format(label), linewidth=4)
        ax.fill_between(range(val_err_mean.shape[0]), val_err_mean - val_err_std, val_err_mean + val_err_std, alpha=0.4, color="{}".format(color))
